...
; Required defines (passed via command line from CMake):
;   CDD_EXE      - Output installer filename
;   CDD_VER      - Version string
;   CDD_EXE_DIR  - Directory containing cd-deluxe.exe
...
; The file to write (can be overridden via /XOutFile on command line)
!ifdef CDD_EXE
    OutFile "${CDD_EXE}"
!else
    OutFile "CddInstaller.exe"
!endif
...
; Add plugin directory for EnVar (current directory contains EnVar.dll)
!addplugindir "."
...
Section "CddInstaller (required)"
  ...
  ; Main executable from build directory
  File "${CDD_EXE_DIR}\cd-deluxe.exe"
  ...
  ; This creates cdd.cmd and cdd.ps1 in the same directory as the executable
  DetailPrint "Generating shell wrapper scripts..."
  nsExec::ExecToLog '"$INSTDIR\cd-deluxe.exe" --init=all --force'
  Pop $0
  ${If} $0 != 0
    DetailPrint "Warning: Wrapper script generation returned code $0"
  ${EndIf}
